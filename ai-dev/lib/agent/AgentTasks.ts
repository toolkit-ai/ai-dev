...
export async function createTaskAgentInput(
  ...
  const prompt = new PromptTemplate({
    template:
      "You're an expert engineer. The codebase you're working with is located in the {workspace_dir} directory. I'm the product manager and need you to implement this task: '{task_description}'. Here's the first 10 files in the directory: {files}. Explore the codebase and complete the task. Make sure applicable tests and linters pass. If the task is unclear, you can ask me via AskHumanTool. {readme} \n{format_instructions}",
